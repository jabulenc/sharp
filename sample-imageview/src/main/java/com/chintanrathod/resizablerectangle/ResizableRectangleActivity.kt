...
import com.pixplicity.sharp.imageviewdemo.databinding.ResizableRectangleActivityBinding
import javax.xml.parsers.SAXParserFactory


class ResizableRectangleActivity : Activity() {
    lateinit var binding: ResizableRectangleActivityBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.resizable_rectangle_activity)
    ...
    override fun onPostResume() {
        ...
        iView.postDelayed(500) {
            ...
            val listOfNewViews = mutableListOf<DrawView>()
            handler.itemMap.forEach { annotationLayer ->
                innerPaths.firstOrNull { it.contains(annotationLayer.key, true) }?.let { ogForm -> annotationLayer.value.originalForm = ogForm }
                listOfNewViews.add(
                    DrawView(this).apply {
                        // TODO: Rather than setting the SVG string, just pass the AnnotationLayer object
                        setSvgString(annotationLayer.value.toDisplayXml(), annotationLayer.key)
                        layoutParams = ConstraintLayout.LayoutParams(binding.actualImage.measuredWidth, binding.actualImage.measuredHeight)
                    }
                )
            }
            val paddingPx = resources.getDimensionPixelSize(R.dimen.ball_rads)
            binding.container.let { container ->
                listOfNewViews.forEach { childView ->
                    val set = ConstraintSet()
                    // set view id, else getId() returns -1
                    // set view id, else getId() returns -1
                    childView.id = childView.svgId.hashCode()
                    container.addView(childView)

                    set.clone(container)
                    set.constrainDefaultHeight(childView.id, ConstraintSet.MATCH_CONSTRAINT_WRAP)
                    set.constrainDefaultWidth(childView.id, ConstraintSet.MATCH_CONSTRAINT_WRAP)
                    set.connect(childView.id, ConstraintSet.TOP, binding.actualImage.id, ConstraintSet.TOP)
                    set.connect(childView.id, ConstraintSet.START, binding.actualImage.id, ConstraintSet.START)
                    set.connect(childView.id, ConstraintSet.END, binding.actualImage.id, ConstraintSet.END)
                    set.connect(childView.id, ConstraintSet.BOTTOM, binding.actualImage.id, ConstraintSet.BOTTOM)
